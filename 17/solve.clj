(ns adamharbin.advent.2023.17.1
...
(defn left-moves [[r c d]]
  (filter #(some? (get-in grid (take 2 %))) [[r (- c 1) :right] [r (- c 2) :right] [r (- c 3) :right]]))

(defn right-moves [[r c d]]
  (filter #(some? (get-in grid (take 2 %))) [[r (+ c 1) :left] [r (+ c 2) :left] [r (+ c 3) :left]]))

(defn up-moves [[r c d]]
  (filter #(some? (get-in grid (take 2 %))) [[(- r 1) c :down] [(- r 2) c :down] [(- r 3) c :down]]))

(defn down-moves [[r c d]]
  (filter #(some? (get-in grid (take 2 %))) [[(+ r 1) c :up] [(+ r 2) c :up] [(+ r 3) c :up]]))
